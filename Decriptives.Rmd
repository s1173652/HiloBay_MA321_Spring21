...
>>>>>>> Stashed changes
```{r}
outcomes <- as.numeric(hilomodified$saltppt)<25 # Created a vector of True and False values where salinity is greater than 25
index.lt25 <- which(outcomes==TRUE)
# Created a vector of the index number where this was true
poss.storms <- hilomodified[index.lt25,]
poss.storms
# Put the rows from hilomodified that made this true into a new data frame
```


```{r}
outcomes.35 <- as.numeric(hilomodified$saltppt)<35 
# Created a vector of True and False values where salinity is greater than 35
index.lt35 <- which(outcomes==TRUE)
# Created a vector of the index number where this was true
poss.storms35 <- hilomodified[index.lt35,]
poss.storms35
# Put the rows from hilomodified that made this true into a new data frame
...
storms <- ggplot(hilomodified[begin,],  aes(x = date, y = as.numeric(cms))) + 
  geom_line(color="black")+
  geom_line(aes(y = as.numeric(saltppt)), color = "red") +
  geom_line(aes(y = as.numeric(Corr.NTU)), color="grey69") +
  geom_line(aes(y=as.numeric(ChlRFU)),color="khaki")

print(storms + ggtitle("Storms")+labs(x="Time"))

```

```{r}

storm.1.7.13 <- ggplot(hilomodified[1000:1500,],  aes(x = date, y = as.numeric(cms))) + 
  geom_line(color="black")+
  geom_line(aes(y = as.numeric(saltppt)), color = "lightsteelblue1") +
  geom_line(aes(y = as.numeric(Corr.NTU)), color="grey69") +
  geom_line(aes(y=as.numeric(ChlRFU)),color="khaki")
...
#library(tseries)
adf.test(hilomodified$cms, alternative = "stationary", k = 0)
